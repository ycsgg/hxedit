...
- 固定 seed fuzz 入口是 `cargo test --test insert_mode deterministic_mixed_document_edit_fuzz_matches_reference_model`。App 层宏 / 普通编辑交错回归入口是 `cargo test app::tests::edit_commands::source_macro_mixed_with_normal_edits_fuzz_matches_reference_model`。新增 fuzz 操作时必须同步更新参考模型语义、错误路径断言和每步后的 invariant 检查；详细规则见 `docs/workflow.md`
