...
- `:re` 使用与 `:s` 相同的 `[mode]<delim><needle><delim><replacement><delim>` 解析（旧 `hex/ascii <needle> -> <replacement>` 仍兼容），默认只能做等长 replacement；命中数超过 65535 时要求用户用 `--force` 二次确认，确认后按批次扫描 / 应用，clean 连续命中走 compact bulk undo + `ReplacementStore` pattern range overlay，dirty/复杂 run 可退回 per-byte undo；`:re!` 只在 needle / replacement 长度不同时允许 real delete + insert，等长 `:re!` 仍必须走 replacement-only 路径；真正变长的 `:re!` 也必须先用同样的 match 上限保护，别把两者写混
