...
- 命令：`:g`、`:hash`、`:xor` / `:xor!`、`:re` / `:re!`、`:fill`、`:zero`、`:export`、`:diff`、`:insp more`、`:dis`、`:sym`、`:data`；`memory` feature 下额外支持 `:mem freeze` / `:mem thaw` 暂停与恢复目标进程，`:mem commit` / `:mem commit-all` 写回 replacement（后者按 VA 升序遍历所有 dirty region），`:w` 等价 `:mem commit` 且 `:w <path>` 被拒绝（改用 `:export`）；`MemorySession` 按 region 持久化未提交 replacement / undo / redo，切换 region 不丢失编辑，`:q` 在任意 region dirty 时拒绝并汇总
- Inspector / side panel：
  - Inspector：ELF、PE/COFF、Mach-O、PNG、ZIP、GZIP、GIF、BMP、WAV、TAR、JPEG
  - Symbol panel：可执行文件 symbol / import 列表与跳转
...
- `:hash` / `logical_bytes()` / `:export`(binary, `for_each_logical_chunk`) / `:fill`·`:zero`(`overwrite_run_positional`) / `:xor!`(`transform_visible_range_in_place`) 都是 piece-walking + 64 KB 分块，不把整段一次性读入内存；单次读长度统一用 `Document::max_contiguous_read_len()`（= `page_size * cache_pages`）裁剪，避免 `read_range` 跨页返回空
...
- ELF 某些头字段可编辑，风险相对可控
- PNG / ZIP 仍不能自动修 CRC、descriptor、目录索引等一致性
- 新增 editable 字段时，宁可少开，也不要把“能改字节”误写成“结构安全”

### 2.5 格式支持深度仍有限

虽然当前已支持 ELF / PE / Mach-O / PNG / ZIP / GZIP / GIF / BMP / WAV / TAR / JPEG，但仍有深度边界：

- ZIP 还没有完整 central directory / ZIP64 / data descriptor 感知
