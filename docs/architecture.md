...
- Remote：可选 `remote-sftp` / `remote-ssh` / `remote-ftp` feature 支持 `--remote sftp://...`、`ssh://...`、`ftp://...` 文件目标；默认 SFTP transport 是系统 OpenSSH SFTP subsystem（复用 SSH config / host-key / key / agent / GSSAPI），`HXEDIT_SFTP_BACKEND=ssh2` 可强制旧 libssh2 后端；`ssh://` 使用 OpenSSH command transport + 远端 `python3` 作为无 SFTP fallback；`ftp://` 使用明文 passive binary FTP；远程 original bytes 通过同一 page-cache / `Document` walker 分块读取，clean remote 的 hash/search/export 使用 streaming fast path，编辑仍只落在 piece table / tombstone / replacement；远程 `:w` 通过远程临时文件 rewrite + fingerprint 冲突检测 + backend rename 覆盖原目标，远程 `:w <path>` 首版拒绝，使用 `:export <path>` 生成本地副本
...
| `src/remote/*` | Remote target 解析、fake remote 测试后端、OpenSSH / libssh2 SFTP 后端、OpenSSH command 后端、FTP 后端 | 不要绕过 `Document` walker；保存必须保留冲突检测和失败不清 dirty 状态 |
...
- 反汇编后端按架构路由（`src/disasm/backend/registry.rs`）：纯 Rust 的 `IcedX86Backend`（x86 / x86_64）与 `YaxpeaxArmBackend`（aarch64），无 C 依赖；arm / riscv64 等暂无后端会返回 `DisassemblyUnavailable`。`yaxpeax-arm` 输出会把 PC-relative `$±0x..` 归一化成绝对 VA，保持与既有 symbolize / 显示管线一致
