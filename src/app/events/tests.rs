...
fn hidden_inspector_focus_falls_back_to_normal_mode() {
    ...
    // Showing a hidden inspector warns when the terminal is too narrow.
    let mut app2 = app_with_inspector_field();
    app2.show_side_panel = false;
    ...
    // Showing a hidden inspector succeeds when width is sufficient.
    let mut app3 = app_with_inspector_field();
    app3.show_side_panel = false;
...
#[test]
fn tab_hides_visible_side_panel_from_normal_focus() {
    let mut app = app_with_inspector_field();
    app.mode = Mode::Normal;
