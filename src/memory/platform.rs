...
mod linux {
    ...
    fn process_fingerprint_for_pid(pid: u32) -> HxResult<ProcessFingerprint> {
        ...
        let start_time = after_comm
            ...
            .nth(19)
            .ok_or_else(|| {
                HxError::MemoryUnavailable(format!("missing /proc/{pid}/stat starttime"))
            })?
            .parse::<u128>()
            .map_err(|err| {
                HxError::MemoryUnavailable(format!("invalid process starttime: {err}"))
            })?;
...
mod tests {
    ...
    fn procfs_backend_lists_current_process_and_reads_self_memory() {
        ...
        let mut backend = open_backend_for_pid(pid).unwrap();
